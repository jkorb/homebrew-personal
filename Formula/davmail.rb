class Davmail < Formula
  ...
  def config_path
    xdg_config = ENV.fetch("XDG_CONFIG_HOME", "#{Dir.home}/.config")
    Pathname.new(xdg_config)/"davmail/davmail.properties"
  end

  service do
    xdg_config = ENV.fetch("XDG_CONFIG_HOME", "#{Dir.home}/.config")
    run [opt_bin/"davmail", "#{xdg_config}/davmail/davmail.properties"]
    keep_alive true
  ...
  def caveats
    <<~EOF
      This formula expects a user config at:

        #{config_path}

      If XDG_CONFIG_HOME is not set, it falls back to ~/.config.
      Start the service after creating your config:
